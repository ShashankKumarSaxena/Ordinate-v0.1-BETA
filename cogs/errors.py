...
class ErrorEvents(commands.Cog):
    ...
    async def on_command_error(self, ctx, error):
        if hasattr(ctx.command, 'on_error'):
            ...
            em = discord.Embed(title=f"{error_em} Error encountered!",description="A bug or error is detected!",color=discord.Color.orange())
            ...
            em.add_field(name="Support Server Link",value=f"Please report it here : [Support Server](https://discord.gg/48Fzjp3KGN)",inline=False)
...
def setup(bot):
    ...
    print(Fore.GREEN + "[STATUS OK] Error cog is ready!")